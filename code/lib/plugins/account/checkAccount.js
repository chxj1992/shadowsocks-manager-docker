...
const checkFlow = (() => {
  var _ref2 = _asyncToGenerator(function* (server, port, startTime, endTime) {
    ...
    try {
      isMultiServerFlow = yield knex('webguiSetting').select().where({ key: 'system' }).then(function (success) {
        if (!success.length) {
    ...
    const flow = yield knex('saveFlow').sum('flow as sumFlow').groupBy('port').select(['port']).where(isMultiServerFlow ? { port } : { id: server, port }).whereBetween('time', [startTime, endTime]);
      ...
      if (a.type >= 2 && a.type <= 5) {
        ...
        if (data.create + data.limit * timePeriod <= Date.now() || data.create >= Date.now()) {
          if (a.autoRemove) {
            ...
            knex('user').delete().where({ id: a.userId }).then();
          }
        }
      }
    });
